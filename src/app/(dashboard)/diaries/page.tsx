...
const visOpts = [{ label: "公开", value: "SHARED" }, { label: "对方可见", value: "PARTNER" }, { label: "仅自己", value: "PRIVATE" }];
...
export default function DiariesPage() {
  ...
  useEffect(() => { let c = false; fetchDiaries().then((r) => { if (!c && r) { setDiaries(r); setIsLoading(false); } }); return () => { c = true; }; }, [fetchDiaries]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold" style={{ fontFamily: "var(--font-serif)" }}>情侣日记</h1>
      ...
      <Dialog onClose={() => setDialogOpen(false)} open={dialogOpen} title={editing ? "编辑日记" : "写日记"}>
        <form className="space-y-4 p-5" onSubmit={handleSave}>
          <Input label="标题" name="title" onChange={(e) => setTitle(e.target.value)} required value={title} />
          <Textarea label="内容" name="content" onChange={(e) => setContent(e.target.value)} placeholder="写下你们的故事..." required value={content} />
          <div className="grid grid-cols-2 gap-4">
            <Select label="可见范围" name="visibility" onChange={(e) => setVisibility(e.target.value)} options={visOpts} value={visibility} />
